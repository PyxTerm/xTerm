...
def _latex_line_begin_table(colwidths, colaligns, booktabs=False, longtable=False):
    alignment = {"left": "l", "right": "r", "center": "c", "decimal": "r"}
    table_columns_fmt = "".join([alignment.get(a, "l") for a in colaligns])
    return "\n".join(
        [
            ("\\begin{table}{" if not longtable else "\\begin{longtable}{")
            + table_columns_fmt
            + "}",
...
def _asciidoc_row(is_header, *args):
    ...
    def make_header_line(is_header, colwidths, colaligns):
        ...
        alignment = {"left": "<", "right": ">", "center": "^", "decimal": ">"}
        # use the column widths generated by Table for the asciidoc column width specifiers
...
_table_formats = {
    ...
    "latex": TableFormat(
        lineabove=_latex_line_begin_table,
    ...
    "latex_booktabs": TableFormat(
        lineabove=partial(_latex_line_begin_table, booktabs=True),
    ...
    "latex_longtable": TableFormat(
        lineabove=partial(_latex_line_begin_table, longtable=True),
        linebelowheader=Line("\\hline\n\\endhead", "", "", ""),
        linebetweenrows=None,
        linebelow=Line("\\hline\n\\end{longtable}", "", "", ""),
        headerrow=_latex_row,
...
Table_formats = list(sorted(_table_formats.keys()))
...
def simple_separated_format(separator):
    ...
    >>> tsv = simple_separated_format("\\t") ; \
        Table([["foo", 1], ["spam", 23]], tablefmt=tsv) == 'foo \\t 1\\nspam\\t23'
    True
...
def _format(val, valtype, floatfmt, intfmt, missingval="", has_invisible=True):
    ...
    >>> hrow = ['\u0431\u0443\u043a\u0432\u0430', '\u0446\u0438\u0444\u0440\u0430'] ; \
        ...
        Table(tbl, headers=hrow) == good_result
    True
...
def _normalize_table_data(table_data, headers, showindex="default"):
    ...
    * pandas.DataFrame (usually used with headers="keys")
    ...
    index = None
    if hasattr(table_data, "keys") and hasattr(table_data, "values"):
        # dict-like and pandas.DataFrame?
        if hasattr(table_data.values, "__call__"):
            # likely a conventional dict
            keys = table_data.keys()
            rows = list(
                izip_longest(*table_data.values())
            )  # columns have to be transposed
        elif hasattr(table_data, "index"):
            # values is a property, has .index => it's likely a pandas.DataFrame (pandas 0.11.0)
            keys = list(table_data)
            if (
                    showindex in ["default", "always", True]
                    and table_data.index.name is not None
            ):
                if isinstance(table_data.index.name, list):
                    keys[:0] = table_data.index.name
                else:
                    keys[:0] = [table_data.index.name]
            vals = table_data.values  # values matrix doesn't need to be transposed
            # for DataFrames add an index per default
            index = list(table_data.index)
            rows = [list(row) for row in vals]
        else:
            raise ValueError("table data doesn't appear to be a dict or a DataFrame")
        ...
        if headers == "keys" and not rows:
            ...
            headers = []
                ...
                headers == "keys"
                and hasattr(table_data, "dtype")
                and getattr(table_data.dtype, "names")
        ):
            # numpy record array
            headers = table_data.dtype.names
        elif (
                headers == "keys"
                and len(rows) > 0
            ...
            rows = [[row.get(k) for k in keys] for row in rows]
                ...
                headers == "keys"
                and hasattr(table_data, "description")
                and hasattr(table_data, "fetchone")
                and hasattr(table_data, "rowcount")
        ):
            # Python Database API cursor object (PEP 0249)
            # print Table(cursor, headers='keys')
            headers = [column[0] for column in table_data.description]
...
def Table(
    ...
    >>> print(Table([["strings", "numbers"], ["spam", 41.9999], ["eggs", "451.0"]],
    ...
    "latex" produces a table environment of LaTeX document markup:

    >>> print(Table([["spam", 41.9999], ["eggs", "451.0"]], tablefmt="latex"))
    \\begin{table}{lr}
    \\hline
     spam &  41.9999 \\\\
     eggs & 451      \\\\
    ...
    >>> print(Table([["spam$_9$", 41.9999], ["\\\\emph{eggs}", "451.0"]], tablefmt="latex_raw"))
    \\begin{table}{lr}
    \\hline
     spam$_9$    &  41.9999 \\\\
     \\emph{eggs} & 451      \\\\
    ...
    "latex_booktabs" produces a table environment of LaTeX document markup
    using the booktabs.sty package:

    >>> print(Table([["spam", 41.9999], ["eggs", "451.0"]], tablefmt="latex_booktabs"))
    \\begin{table}{lr}
    \\toprule
     spam &  41.9999 \\\\
     eggs & 451      \\\\
    ...
    "latex_longtable" produces a table environment that can stretch along
    multiple pages, using the longtable package for LaTeX.

    >>> print(Table([["spam", 41.9999], ["eggs", "451.0"]], tablefmt="latex_longtable"))
    \\begin{longtable}{lr}
    \\hline
     spam &  41.9999 \\\\
     eggs & 451      \\\\
    ...
    list_of_lists, headers, headers_pad = _normalize_table_data(
        table_data, headers, showindex=showindex
    )
...
class _CustomTextWrap(textwrap.TextWrapper):
    ...
    def _wrap_chunks(self, chunks):
        ...
        while chunks:
            ...
            # First chunk online is whitespace -- drop it, unless this
...
def _main():
    """\
    Usage: Table [options] [FILE ...]

    Pretty-print table data.
    See also https://github.com/astanin/python-Table

    FILE                      a filename of the file with table data;
